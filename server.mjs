...
app.get("/api/prepare", (req, res) => {
  ...
  ytDlp.on("close", (code) => {
    if (code === 0 && fs.existsSync(filePath)) {
      ...
      console.log(`[Prepare] Success: ${videoId} -> ${filePath}`);
      ...
      console.error(`[Prepare] Failed for ${videoId} with exit code ${code}`);
      if (stderr) console.error(`[Prepare] yt-dlp error output: ${stderr}`);
      if (fs.existsSync(filePath)) {
